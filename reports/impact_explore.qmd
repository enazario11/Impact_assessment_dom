...
Percent of data where the VSR zone was active vs. inactive, and within VSR status groupings, what the whale presence rating was.
...
PD <- wpr %>%
  group_by(vsr_season, whale_presence_rating) %>% 
  summarise(n = n()) %>%
...
colnames(PD) <- c("VSR_status", "Whale_presence_rating", "count")

PieDonut_c(PD, aes(VSR_status, Whale_presence_rating, count = count), showDonutName = TRUE)
```

## Whale presence rating and vessel cooperation

Percent cooperation grouped by VSR status (inactive vs. active) and whale presence rating
...
#| fig-height: 8
#| fig-align: "center"
#| label: whale presence rating and vessel speed cooperation
...
## Whale watch and blue whale sightings time series
...
#| fig-height: 8
#| fig-align: "center"
#| label: whale watch and vessel speed cooperation
...
sf_sight <- wpr_join %>% 
  ...
  xlab("")+
  labs(title = "SF VSR")+
  ylab("Mean year-month blue whale sightings") +
  ms_theme()
...
sf_ww <- wpr_join %>% 
  ...
  xlab("")+
  ylab("Mean year-month blue whale habitat model") +
  ms_theme()
