...
sf_whale <- wpr_join %>% 
  ...
  geom_path(size = 1.5, alpha = 0.8)+
  xlab("")+
  ylab("Mean year-month whale sightings") +
  labs(title = "SF VSR") +
...
sf_ac <- wpr_join %>% 
  group_by(yr_mo, zone) %>%
  summarise(sight_blu = mean(bw_ac_score, na.rm = TRUE)) %>%
  filter(zone == "SF VSR") %>%
  ggplot(aes(x = yr_mo)) +
  geom_rect(data = sf_vsr_on2, aes(ymin = -Inf, ymax = Inf, xmin = yr_mo_s, xmax = yr_mo_e), inherit.aes = FALSE, fill = "grey", alpha = 0.5)+
  geom_point(aes(y = sight_blu), size = 3, color = "#1F2D3D")+
  geom_path(aes(y = sight_blu), size = 1.5, alpha = 0.8, color = "#1F2D3D")+
  xlab("")+
  scale_y_continuous(name = "Acoustics rating", 
                     breaks = c(1, 2, 3),
                     labels = c("Low Rating", "Medium Rating", "High Rating"), 
                     limits = c(0.7, 3.3))+
  ms_theme()
...
wpr_join %>% 
  ...
  group_by(mo, year, zone) %>%
  summarise(mean_co = mean(s_bluecount)) %>%
  ggplot(aes(x = as.factor(mo), y = mean_co, group = as.factor(year)))+
  geom_path(aes(color = year), size = 1, alpha = 0.4)+
  geom_point(aes(color = year), size = 4)+
  xlab("") +
  ylab("Blue whale sightings (No. whales)")+
  scale_color_manual(values = met.brewer("Hokusai2", n = 5)) +
  scale_fill_manual(values = met.brewer("Hokusai2", n = 5)) + 
  facet_wrap(~zone)+
  ms_theme()
...
wpr_join %>% 
  distinct(id, .keep_all = TRUE) %>%
  filter(s_bluecount < 5 & zone == "Socal VSR" & year != 2025) %>%
  ggplot(aes(x = s_bluecount, y = as.factor(mo), fill = stat(x))) +
...
wpr_join %>% 
  ...
  mutate(year = as.factor(year)) %>%
  group_by(mo, year, zone) %>%
  summarise(mean_co = mean(bw_ac_score)) %>%
  ggplot(aes(x = as.factor(mo), y = mean_co, group = as.factor(year)))+
  geom_path(aes(color = year), size = 1, alpha = 0.4)+
  geom_point(aes(color = year), size = 4)+
  xlab("") +
...
wpr_join %>% 
  distinct(id, .keep_all = TRUE) %>%
  filter(zone == "Socal VSR") %>%
  ggplot(aes(x = bw_ac_score, y = as.factor(mo), fill = stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
...
wpr_join %>% 
  distinct(id, .keep_all = TRUE) %>%
  filter(zone == "Socal VSR") %>%
  ggplot(aes(x = bwmv, y = as.factor(mo), fill = stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
...
```{r}
#| warning: false
#| label: blue whale data stream comparison corr test, pear
...
```{r}
#| warning: false
#| label: blue whale data stream comparison corr test, spear
...
```{r}
#| warning: false
#| label: blue whale data stream comparison corr test, ken
