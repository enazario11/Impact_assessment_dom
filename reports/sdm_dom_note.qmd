...
wpr <- wpr %>% mutate(j_day = format(date_pt, "%j"), 
                      ...
                      blu_acou_deci = ifelse(blu_acou_deci == 0, 0.00001, 
                                                ifelse(blu_acou_deci == 1, 0.99999, blu_acou_deci)))

#sightings ~ sdm
sight_sdm <- glmmTMB(s_bluecount ~ bwmv + zone + mo + (1|year), 
                    ziformula = ~1,
                    family = poisson,
                    data = wpr) #also consider day of year and year covars
...
# Vary the variable of interest (e.g., bwmv) while holding others constant
pred_data <- expand.grid(
  bwmv = seq(min(wpr$bwmv, na.rm = TRUE), max(wpr$bwmv, na.rm = TRUE), length.out = 100),
  zone = unique(wpr$zone),   # hold at reference level
  mo   = unique(wpr$mo),     # hold at reference level
  year = NA                  # NA = population-level (exclude random effect)
)

# Predict on the response scale (counts), excluding random effects
...
#SF
cor.test(sf_corr$bwmv, sf_corr$blue_acoustic_percent, method = "spearman", use = "complete.obs")

sum(complete.cases(sf_corr$bwmv, sf_corr$blue_acoustic_percent))
se.spear(0.68, 290)

#Socal
cor.test(socal_corr$bwmv, socal_corr$blue_acoustic_percent, method = "spearman", use = "complete.obs")

sum(complete.cases(socal_corr$bwmv, socal_corr$blue_acoustic_percent))
se.spear(0.19, 1089)
...
#SF
cor.test(sf_corr$bwmv, sf_corr$blue_acoustic_percent, method = "kendall", use = "complete.obs")

n = sum(complete.cases(sf_corr$bwmv, sf_corr$blue_acoustic_percent))
sqrt((2 * (2 * n + 5)) / (9 * n * (n - 1)))


#Socal
cor.test(socal_corr$bwmv, socal_corr$blue_acoustic_percent, method = "kendall", use = "complete.obs")

n = sum(complete.cases(socal_corr$bwmv, socal_corr$blue_acoustic_percent))
sqrt((2 * (2 * n + 5)) / (9 * n * (n - 1)))
...
#SF
cor.test(sf_corr$bwmv, sf_corr$s_bluecount, method = "spearman", use = "complete.obs")

sum(complete.cases(sf_corr$bwmv, sf_corr$s_bluecount))
se.spear(0.249, 678)

#Socal
cor.test(socal_corr$bwmv, socal_corr$s_bluecount, method = "spearman", use = "complete.obs")

sum(complete.cases(socal_corr$bwmv, socal_corr$s_bluecount))
se.spear(0.16, 1402)
...
#SF
cor.test(sf_corr$bwmv, sf_corr$s_bluecount, method = "kendall", use = "complete.obs")

n = sum(complete.cases(sf_corr$bwmv, sf_corr$s_bluecount))
sqrt((2 * (2 * n + 5)) / (9 * n * (n - 1)))


#Socal
cor.test(socal_corr$bwmv, socal_corr$s_bluecount, method = "kendall", use = "complete.obs")

n = sum(complete.cases(socal_corr$bwmv, socal_corr$s_bluecount))
sqrt((2 * (2 * n + 5)) / (9 * n * (n - 1)))
