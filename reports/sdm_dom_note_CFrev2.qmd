...
#sightings ~ sdm
sight_sdm <- glmmTMB(s_bluecount ~ bwmv:zone + mo + year_c, 
                    ziformula = ~1,
                    family = poisson,
                    data = wpr)
...
exp(fixef(sight_sdm)$cond["(Intercept)"]) #baseline for SF VSR at mean year, mean month, and bwmv = 0
...
#acoustics ~ sdm
acou_sdm <- glmmTMB(blu_acou_deci ~ bwmv:zone + mo + year_c, 
                    ziformula = ~zone,
