...
#sightings ~ sdm
sight_sdm <- glmmTMB(s_bluecount ~ bwmv:zone + mo + (1|year_c), 
                    ziformula = ~1,
                    family = poisson,
                    data = wpr)
...
#acoustics ~ sdm
acou_sdm <- glmmTMB(blu_acou_deci ~ bwmv:zone + mo + (1|year_c), 
                    ziformula = ~zone,
...
plogis(fixef(acou_sdm)$cond["(Intercept)"]) #baseline for SF VSR at mean year, mean month, and bwmv = 0
...
acou_tidy <- tidy(acou_sdm, effects = "fixed", component = "cond") %>%
  ...
  rename(acou_est = estimate, acou_sig = sig) %>%
  select(-p.value)

sight_tidy <- tidy(sight_sdm, effects = "fixed", component = "cond") %>%
  select(term, estimate, p.value) %>%
  mutate(
    sig = case_when(
      p.value < 0.001 ~ "***",
  ...
  rename(sight_est = estimate, sight_sig = sig) %>%
  select(-p.value)

left_join(acou_tidy, sight_tidy, by = "term") %>%
  print()
```