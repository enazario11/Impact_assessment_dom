...
```{r}
#| warning: false
#| label: vis sight raw data

ggplot(wpr, aes(bwmv, s_bluecount)) + 
geom_point(aes(color = zone)) + 
geom_smooth(data = wpr, aes(bwmv, s_bluecount), color = "black", method = "lm") + 
...
wpr$year_c <- wpr$year - mean(wpr$year) #center to make intercept value more interpretable
wpr$zone <- relevel(wpr$zone, ref = "Socal VSR")
n_sight_mod <- wpr %>% filter(!is.na(bwmv)) %>% group_by(zone) #no NAs in sighting data
count(n_sight_mod)

#sightings ~ sdm
sight_sdm <- glmmTMB(s_bluecount ~ bwmv*zone + month_sin + month_cos + (1|year_c), 
                    ziformula = ~zone,
                    family = poisson,
                    data = wpr)
...
For the sightings data, we see a marginally non-significant difference between zones (0.0792) suggesting that the relationship between WhaleWatch and sightings may differ between zones. We do see that WhaleWatch has a positive significant relationship with sightings data in Socal (our reference level) and when HSI = 1, we expect a 2.6x increase in whales compared to when HSI = 0.

```{r}
#| warning: false
#| label: interpret socal sight sdm output

exp(fixef(sight_sdm)$cond)
emtrends(sight_sdm, pairwise ~ zone, var = "bwmv")

exp(confint(sight_sdm, parm = "beta_")[, 1:2])
```
...
```{r}
#| warning: false
#| label: vis acou raw data

ggplot(wpr, aes(bwmv, blu_acou_deci)) + 
geom_point(aes(color = zone)) + 
geom_smooth(data = wpr, aes(bwmv, blu_acou_deci), color = "black", method = "lm", se = FALSE) + 
geom_smooth(method = "lm", aes(color = zone), se = FALSE) + 
theme_minimal()
```
...
n_acou_mod <- wpr %>% filter(!is.na(blu_acou_deci)) %>% filter(!is.na(bwmv)) %>% group_by(zone)
count(n_acou_mod, "zone")

#acoustics ~ sdm
acou_sdm <- glmmTMB(blu_acou_deci ~ bwmv*zone + month_sin + month_cos + (1|year_c), 
                    ziformula = ~zone,
...
For the acoustics data, we see significant difference between zones (p \< 0.0001) suggesting that the relationship between WhaleWatch and summary periods with positive acoustic detections likely differ between zones. We do see that WhaleWatch has a positive significant relationship with sightings data in Socal (our reference level) and when HSI = 1, we expect a 0.053x increase in whales compared to when HSI = 0.

```{r}
#| warning: false
#| label: interpret acou outputs

#two approaches to transforming coefficient estimates on response scale
ggeffects::predict_response(acou_sdm, terms = "zone")
emmeans(acou_sdm, ~ bwmv|zone, type = "response")

#predicted values across interaction terms and combinations
interaction_preds <- ggeffects::predict_response(acou_sdm, terms = c("bwmv", "zone"))
plot(interaction_preds)


plogis(fixef(acou_sdm)$cond)*100 #baseline for SF VSR at mean year, mean month, and bwmv = 0

emtrends(acou_sdm, pairwise ~ zone, var = "bwmv")

plogis(confint(acou_sdm, parm = "beta_")[, 1:2])*100
